...
from oficina where oficina.pais ='España';

# 3 punto
select empleado.nombre, empleado.apellido1, empleado.apellido2, empleado.email, empleado.codigo_jefe
from empleado
where empleado.codigo_jefe = 7;

# 4 punto
select distinct pago.codigo_cliente, pago.fecha_pago
from pago
where year(fecha_pago) = 2008;

# 5 punto
select count(empleado.codigo_empleado) as cantidad_empleados
from empleado;

# 6 punto
select cliente.pais, count(cliente.codigo_cliente) as cantidad_clientes
from cliente
group by cliente.pais;

# 7 punto
select avg(pago.total) as pago_medio
from pago
where year(pago.fecha_pago) = 2009;

# 8 punto
select pedido.estado, count(pedido.codigo_pedido) as cantidad_pedidos
from pedido
group by pedido.estado
order by cantidad_pedidos desc;

# 9 punto 
select max(producto.precio_venta) as maximo_precio , min(producto.precio_venta) as minimo_precio
from producto;

# 10 punto
select cliente.nombre_cliente, max(cliente.limite_credito)
from cliente
group by cliente.codigo_cliente limit 1;

# 11 punto
select producto.nombre, producto.precio_venta
from producto
order by producto.precio_venta desc limit 1;

# 12 punto
select producto.nombre, detalle_pedido.codigo_producto, sum(detalle_pedido.cantidad)
from detalle_pedido
inner join producto on detalle_pedido.codigo_producto = producto.codigo_producto
group by detalle_pedido.codigo_producto
order by sum(detalle_pedido.cantidad) desc limit 1;

# 13 punto
select  cliente.nombre_cliente, cliente.limite_credito, pago.codigo_cliente, sum(total)
from pago, cliente
...
# 14 punto
select cliente.nombre_cliente, sum(pedido.codigo_cliente) as pedidos
from pedido
right join cliente
...
where empleado.codigo_empleado not in (
	select distinct cliente.codigo_empleado_rep_ventas
    from cliente
) and empleado.codigo_oficina = oficina.codigo_oficina
order by puesto;

# 16 punto
select oficina.codigo_oficina, oficina.ciudad, oficina.telefono
from oficina
...
#17 punto
select cliente.nombre_cliente, count(pedido.codigo_cliente) as pedidos
from pedido
right join cliente
on pedido.codigo_cliente = cliente.codigo_cliente
group by cliente.nombre_cliente
